...
const Login = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        // We bypass the API entirely as requested
        try {
            // Determine role purely by email convention for demo purposes
            const email = loginId.toLowerCase();
            if (email.includes('admin')) {
                // Should use Admin Login page, but if they try here, redirect them
                navigate('/admin/login');
                return;
            } else if (email.includes('vendor')) {
                // Should use Vendor Login
            ...
            console.error(err);
            setError('Demo login failed locally');
        } finally {
            setLoading(false);
        }
    };
