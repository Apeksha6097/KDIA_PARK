// REVERTED FROM MYSQL TO SQLITE (2026-02-03)
// This file was restored from legacy/database.js to re-enable SQLite database
// MySQL configuration is disabled (see legacy/kdia_portal.sql for backup)
...
async function initDB() {
  ...
  const newColumns = [
    ...
    { name: 'lockUntil', type: 'INTEGER DEFAULT 0' }
  ];
  ...
  if (!ticketColumns.includes('priority')) {
    await db.exec("ALTER TABLE support_tickets ADD COLUMN priority TEXT DEFAULT 'LOW'");
    console.log("Migration: Added priority column to support_tickets");
  }

  if (!ticketColumns.includes('discom')) {
    await db.exec("ALTER TABLE support_tickets ADD COLUMN discom TEXT");
    console.log("Migration: Added discom column to support_tickets");
  }

  if (!ticketColumns.includes('park_district')) {
    await db.exec("ALTER TABLE support_tickets ADD COLUMN park_district TEXT");
    console.log("Migration: Added park_district column to support_tickets");
  }

  if (!ticketColumns.includes('assigned_support_id')) {
    await db.exec("ALTER TABLE support_tickets ADD COLUMN assigned_support_id INTEGER");
    console.log("Migration: Added assigned_support_id column to support_tickets");
  }

  if (!ticketColumns.includes('updatedAt')) {
    await db.exec("ALTER TABLE support_tickets ADD COLUMN updatedAt DATETIME DEFAULT CURRENT_TIMESTAMP");
    console.log("Migration: Added updatedAt column to support_tickets");
  }

  if (!ticketColumns.includes('revokedAt')) {
    await db.exec("ALTER TABLE support_tickets ADD COLUMN revokedAt DATETIME");
    console.log("Migration: Added revokedAt column to support_tickets");
  }
