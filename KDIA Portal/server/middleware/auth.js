...
const JWT_SECRET = process.env.JWT_SECRET || 'kdia-dev-fallback-key-safe-for-local-only';
...
const supportMiddleware = (req, res, next) => {
    if (req.userRole !== 'support_agent' && req.userRole !== 'admin') {
        return res.status(403).json({ error: "Access denied: Support Team only" });
    }
    next();
};

module.exports = { authenticateToken, adminMiddleware, supportMiddleware };
