...
function initSavingsCalculator() {
    ...
    function updateCalculations() {
        ...
        if (annualSavings > 0 && principal > 0) {
            ...
            if (roiValue) {
                roiValue.textContent = `${payback}-${payback + 2}`;
            }
        }
    }
    ...
    if (tenureSlider) {
        tenureSlider.addEventListener('input', () => {
            if (tenureDisplay) {
                const isHindi = document.documentElement.getAttribute('lang') === 'hi';
                tenureDisplay.textContent = tenureSlider.value + (isHindi ? ' वर्ष' : ' Years');
            }
        });
    }
...
const ApplicationModal = {
    ...
    showStep(step) {
        ...
        if (this.nextBtn) {
            const isHindi = document.documentElement.getAttribute('lang') === 'hi';
            const btnText = step === 3 
                ? (isHindi ? 'आवेदन जमा करें' : 'Submit Application') 
                : (isHindi ? 'अगला' : 'Next');
            this.nextBtn.innerHTML = `${btnText} <i data-lucide="chevron-right" class="w-4 h-4"></i>`;
        }
        if (typeof lucide !== 'undefined') lucide.createIcons();
    },
