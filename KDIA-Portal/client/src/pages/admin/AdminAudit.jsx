...
const AdminAudit = () => {
    ...
    useEffect(() => {
        const fetchLogs = async () => {
            try {
                const localLogs = JSON.parse(localStorage.getItem('kdia_audit_logs') || '[]');
                let apiLogs = [];
                try {
                    ...
                    console.warn("API audit logs fetch failed, using local storage only");
                }
                
                // Merge and deduplicate by id
                const merged = [...localLogs, ...apiLogs];
                const uniqueLogs = Array.from(new Map(merged.map(item => [item.id, item])).values());
                
                setLogs(uniqueLogs);
                setLoading(false);
            } catch (err) {
                setError('Failed to load administrative audit logs.');
                setLoading(false);
            }
        };
        fetchLogs();
    ...
    const processedLogs = React.useMemo(() => {
        ...
        if (searchTerm) {
            ...
            filtered = filtered.filter(log => {
                ...
                const targetStr = (log.targetUser || '').toLowerCase();
                ...
                return timestampStr.includes(lowerSearch) ||
                    ...
                    targetStr.includes(lowerSearch) ||
                    roleStr.includes(lowerSearch) ||
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                    <thead>
                        <tr style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
                            ...
                            >
                                Date & Time <span style={{ marginLeft: '4px', opacity: sortConfig.key === 'timestamp' ? 1 : 0.3 }}>{getSortIcon('timestamp')}</span>
                            </th>
                            <th
                            ...
                            <th
                                ...
                                style={{ padding: '16px 24px', fontWeight: '700', color: '#475569', cursor: 'pointer', userSelect: 'none', transition: 'background 0.2s' }}
                            >
                                Performed By <span style={{ marginLeft: '4px', opacity: sortConfig.key === 'adminName' ? 1 : 0.3 }}>{getSortIcon('adminName')}</span>
                            </th>
                            <th style={{ padding: '16px 24px', fontWeight: '700', color: '#475569' }}>Target User/Entity</th>
                            <th style={{ padding: '16px 24px', fontWeight: '700', color: '#475569' }}>Role</th>
                            <th style={{ padding: '16px 24px', fontWeight: '700', color: '#475569' }}>Status/Result</th>
                        </tr>
                    </thead>
                    <tbody>
                        {processedLogs.map((log) => (
                            <tr key={log.id} style={{ borderBottom: '1px solid #f1f5f9', fontSize: '0.9rem', transition: 'background 0.2s' }}>
                                ...
                                <td style={{ padding: '14px 24px', color: '#4a5568' }}>{log.targetUser || formatDetails(log.details) || 'N/A'}</td>
                                <td style={{ padding: '14px 24px', color: '#4a5568' }}>{log.role || 'N/A'}</td>
                                <td style={{ padding: '14px 24px', color: '#4a5568' }}>
                                    <span style={{
                                        padding: '4px 10px',
                                        borderRadius: '8px',
                                        background: log.status === 'Success' || log.status === 'Active' ? '#dcfce7' : log.status === 'Inactive' ? '#fee2e2' : '#f1f5f9',
                                        color: log.status === 'Success' || log.status === 'Active' ? '#166534' : log.status === 'Inactive' ? '#991b1b' : '#475569',
                                        fontSize: '0.75rem',
                                        fontWeight: '800',
                                        border: `1px solid ${log.status === 'Success' || log.status === 'Active' ? '#bbf7d0' : log.status === 'Inactive' ? '#fecaca' : '#e2e8f0'}`
                                    }}>
                                        {log.status || 'N/A'}
                                    </span>
                                </td>
