...
const AdminVendors = () => {
    ...
    const fetchVendors = async () => {
        try {
            ...
            // Load locally created vendors
            const createdVendorsStr = localStorage.getItem('kdia_created_vendors');
            const createdVendors = createdVendorsStr ? JSON.parse(createdVendorsStr) : [];
            
            // Load status overrides
            const overridesStr = localStorage.getItem('kdia_vendor_status_overrides');
            const overrides = overridesStr ? JSON.parse(overridesStr) : {};
            
            // Apply overrides and merge
            const apiVendors = response.data.map(v => ({
                ...v,
                isActive: overrides[v.id] !== undefined ? overrides[v.id] : v.isActive
            }));
            
            setVendors([...createdVendors, ...apiVendors]);
            setLoading(false);
        } catch (err) {
            ...
            const merged = [...createdVendors, ...mockVendors].map(v => ({
                ...v,
                isActive: overrides[v.id] !== undefined ? overrides[v.id] : v.isActive
            }));
            
            setVendors(merged);
            setError(''); // Clear error to show fallback gracefully
            setLoading(false);
        }
    };
    ...
    const handleToggleStatus = async (vendorId, currentActive) => {
        ...
        // Find the vendor's email to log it
        const vendor = vendors.find(v => v.id === vendorId);
        const email = vendor ? vendor.email : 'unknown@vendor.com';
        
        // Log to audit log
        logAdminAction(newActiveState ? 'Vendor Activated' : 'Vendor Deactivated', email, 'Vendor', 'Success');
        logAdminAction('User Status Changed', email, 'Vendor', newActiveState ? 'Active' : 'Inactive');
        
        // 1. Update localStorage overrides
        const overridesStr = localStorage.getItem('kdia_vendor_status_overrides');
        const overrides = overridesStr ? JSON.parse(overridesStr) : {};
        overrides[vendorId] = newActiveState;
        localStorage.setItem('kdia_vendor_status_overrides', JSON.stringify(overrides));

        
        // If it's a locally created vendor, update it in kdia_created_vendors list too
        const createdVendorsStr = localStorage.getItem('kdia_created_vendors');
        if (createdVendorsStr) {
            const createdVendors = JSON.parse(createdVendorsStr);
            const updated = createdVendors.map(v => v.id === vendorId ? { ...v, isActive: newActiveState } : v);
            localStorage.setItem('kdia_created_vendors', JSON.stringify(updated));
        }
        
        // 2. Update React state immediately
        setVendors(prev => prev.map(v => v.id === vendorId ? { ...v, isActive: newActiveState } : v));
        
        // 3. Show success Toast
        setToast({ 
            message: `Vendor account ${newActiveState ? 'activated' : 'deactivated'} successfully.`, 
            type: 'success' 
        });
        
        // 4. Try hitting backend API
        try {
            await api.patch(`/admin/vendors/${vendorId}/status`, { isActive: newActiveState }, {
                headers: { Authorization: `Bearer ${token}` }
            });
        } catch (err) {
            console.info("Using offline localStorage fallback for vendor status change.");
        }
    };
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                    <thead>
                        <tr style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
                            ...
                            <th style={{ padding: '18px 24px', fontWeight: '700', color: '#64748b', fontSize: '0.8rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Account Status</th>
                            <th style={{ padding: '18px 24px', fontWeight: '700', color: '#64748b', fontSize: '0.8rem', textTransform: 'uppercase', letterSpacing: '0.05em', textAlign: 'right' }}>Actions</th>
                        </tr>
                    </thead>
                                ...
                                >
                                    ...
                                    <td style={{ padding: '20px 24px', textAlign: 'right' }}>
                                        <button
                                            ...
                                            style={{
                                                background: vendor.isActive ? '#fee2e2' : '#d1fae5',
                                                color: vendor.isActive ? '#b91c1c' : '#065f46',
                                                border: 'none',
                                                padding: '8px 12px',
                                                borderRadius: '6px',
                                                cursor: 'pointer',
